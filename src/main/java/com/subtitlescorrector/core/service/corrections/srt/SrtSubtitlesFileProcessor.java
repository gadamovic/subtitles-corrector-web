...
@Component
public class SrtSubtitlesFileProcessor {
	...
	@Autowired
	SrtFileSubtitleDataToLinesForAiCorrectionAdapter aiAdapter;
	...
	public SrtSubtitleFileData process(File storedFile, List<String> lines, AdditionalData params, BomData bomData) {
		...
		SrtSubtitleFileData data = new SrtSubtitleFileData();
		...
		try {
			...
			for(SrtSubtitleUnitData subUnit : data.getLines()) {
	...
	private void populateSubtitleFileData(AdditionalData params, SrtSubtitleFileData data, File subtitleFile,
	...
	private void calculateEditOperationsAfterCorrections(SrtSubtitleFileData data) {
		for(SrtSubtitleUnitData subData : data.getLines()) {
